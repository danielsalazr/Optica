async function addRow() {
    ...
    row.innerHTML = `
        ...
        <td>
            <select class="form-select border-0" name="numero_articulo" id="articulo${table.rows.length+1}">
                <option value="">Seleccione</option>
                ${articulos.data.articulos.map( (item) => (`
        ...
        <td><input type="number" class="form-control" name="precio" placeholder="Precio" step="0.01"></td>
        <td>
    ...
    const selector = $(`#articulo${table.rows.length}`).selectize({
        //maxOptions: 5,
      })
